	...
	m_pipelineStateShadowMap(pShadowMapPso)
{
	...
	for (UINT i = 0; i < gNumContexts/*NumContexts*/; i++)
	{
	...
	const UINT batchSize = gNumContexts * 2 + 3;
	...
	memcpy(m_batchSubmit + 1, m_shadowCommandLists, gNumContexts * sizeof(ID3D12CommandList*));
	//m_batchSubmit[_countof(m_shadowCommandLists) + 1] = m_commandLists[CommandListMid].Get();
	m_batchSubmit[gNumContexts + 1] = m_commandLists[CommandListMid].Get();
	...
	memcpy(m_batchSubmit + gNumContexts + 2, m_sceneCommandLists, gNumContexts * sizeof(ID3D12CommandList*));
