...
void D3D12Multithreading::OnRender()
{
...
#if SINGLETHREADED
	for (int i = 0; i < gNumContexts/*NumContexts*/; i++)
	{
		WorkerThread(i);
	}
	MidFrame();
	...
	WaitForMultipleObjects(gNumContexts/*NumContexts*/, m_workerFinishShadowPass, TRUE, INFINITE);
	...
	m_commandQueue->ExecuteCommandLists(gNumContexts/*NumContexts*/ + 2, m_pCurrentFrameResource->m_batchSubmit); // Submit PRE, MID and shadows.

	WaitForMultipleObjects(gNumContexts/*NumContexts*/, m_workerFinishedRenderFrame, TRUE, INFINITE);

	// Submit remaining command lists.
	m_commandQueue->ExecuteCommandLists(_countof(m_pCurrentFrameResource->m_batchSubmit) - gNumContexts/*NumContexts*/ - 2, m_pCurrentFrameResource->m_batchSubmit + gNumContexts/*NumContexts*/ + 2);
#endif
...
void D3D12Multithreading::WorkerThread(int threadIndex)
{
	...
	assert(threadIndex < gNumContexts/*NumContexts*/);
#if !SINGLETHREADED

	while (threadIndex >= 0 && threadIndex < gNumContexts/*NumContexts*/)
	{
		...
		for (int j = threadIndex; j < _countof(SampleAssets::Draws); j += gNumContexts/*NumContexts*/)
		{
